...
- **훈련 (플래시카드 세션)**: 덱(전체 / 오픈 / 오픈 대응 / 3벳 대응 / 4벳·올인 / 내 약점)과 포지션을 고르면 10·20·40장 세션이 시작됩니다. 핸드가 나오면 **폴드 · 콜 · 3벳 같은 액션 버튼**으로 직접 고르고, 고르는 즉시 타이머가 멈추고 정답과 한 줄 해설이 나오며(다음 카드는 **다음** 버튼으로 넘어갑니다) 화면 배경이 정답(청록) / 오답(호박) / 시간 초과(남색)로 바뀝니다. 생각 시간은 숫자로 보이고(보통 8초, 천천히 12초, 빠르게 5초), 답을 고른 뒤에는 타이머가 멈추고 다음 버튼으로 넘어가요. 답 N초 자동 진행은 노출·순간기억 모드에만 적용돼요. 시간 안에 못 고르면 시간 초과로 기록됩니다. **순간기억(1.5초)** 프리셋과 답을 처음부터 같이 보는 **노출 모드**는 선택 없이 훑어보기만 합니다. 화면을 **길게 누르면 타이머가 멈추고 해설**이 나옵니다. 정답이면 알아요, 오답·시간 초과면 헷갈려요로 간격 반복(SRS) 큐에 반영되어 다음 세션에 복습으로 돌아오고, 맞혔지만 자신 없는 카드는 "헷갈려요로 표시"로 바꿀 수 있습니다. 세션이 끝나면 요약(정답 비율, 약점 상황, 틀린 카드)과 함께 "틀린 것만 다시", "퀴즈로 확인"을 고를 수 있습니다.
