...
처음에는 L1(복사)과 L2(내 키)가 **같은 시스템 프롬프트**를 썼고, 그 프롬프트가 JSON 배열만
출력하라고 시켰다. L2 는 `parseCoachCards` 가 그걸 먹으니 맞다. 그런데 L1 은 사용자가 복사해서
Claude·ChatGPT 화면에 붙여넣고 **거기서 눈으로 읽는** 답이다. 앱으로 돌아오는 길이 없다
(붙여넣기 왕복은 일부러 강요하지 않기로 했다). 파서가 없는 자리에 JSON 을 시키면 읽기만 나빠지고,
카드 상자가 없으니 20자·25자 같은 길이 제한도 뜻이 없다.

→ 규칙(역할·금지어·문장 규칙)은 `COACH_SYSTEM_RULES` 로 함께 쓰고, 출력 형식만 갈랐다.

| | L1 복사 | L2 내 키 |
|---|---|---|
| 상수 | `COACH_COPY_PROMPT` | `COACH_SYSTEM_PROMPT` |
| 형식 | 한국어 평문. 소제목·굵게·목록 허용, 800자 안팎 | JSON 배열 4개 |
| 길이 제한 | 없음(상자가 없다) | title 20자 / evidence 30자 / steps 각 25자 |
| 끝맺음 | "더 궁금한 게 있으면 물어보세요" — 채팅이라 이어서 물을 수 있다 | 없음(카드로 렌더된다) |

개념적 조언이라는 본래 제약(차트 재설명 금지, 금지어, 핸드 단위 단정 금지, 질문으로 끝내기)은
형식과 무관하므로 양쪽에 똑같이 건다. 테스트가 그 대칭을 지킨다.
