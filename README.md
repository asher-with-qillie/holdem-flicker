...
- **훈련 (플래시카드 세션)**: 덱(전체 / 오픈 / 오픈 대응 / 3벳 대응 / 4벳·올인 / 내 약점)과 포지션을 고르면 10·20·40장 세션이 시작됩니다. 핸드가 나오고 잠시 뒤 정답 액션이 표시되며 자동으로 다음 카드로 넘어갑니다. 속도는 천천히·보통·빠르게·**순간기억(약 0.9초)** 프리셋, 답을 처음부터 같이 보는 **노출 모드**도 있습니다. 화면을 **길게 누르면 타이머가 멈추고 해설**이 나옵니다. 답을 본 뒤 **오른쪽 스와이프 = 알아요, 왼쪽 = 헷갈려요**로 평가하면 간격 반복(SRS) 큐에 반영되어 다음 세션에 복습으로 돌아옵니다. 세션이 끝나면 요약(알아요 비율, 약점 상황, 헷갈린 카드)과 함께 "헷갈린 것만 다시", "퀴즈로 확인"을 고를 수 있습니다.
