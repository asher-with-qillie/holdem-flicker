...
/** 액션 단정 어미. 손패 이름과 같은 줄에 있으면 "AJo는 UTG에서 폴드입니다"가 됩니다. */
const VERDICT = /(?:폴드|콜|오픈|3벳|4벳|5벳|올인)(?:입니다|이에요|예요|하세요|합니다)/;
...
function lintField(f: Field): string | null {
  ...
  // 핸드 단위 단정. 기계가 잡을 수 있는 최소 형태로만 봅니다 — 한 줄 안에 손패 이름과
  // 액션 단정이 같이 있으면 그건 차트가 할 말이지 코치가 할 말이 아닙니다.
